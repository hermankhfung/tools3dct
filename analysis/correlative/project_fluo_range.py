...
'''Calculate projection of 3D volumes based on 3DCT parameters and the selected mask series

Run 'python3 project_fluo_range.py --help' for command line usage information.

All masks in the same directory with the same prefix as the selected mask and
syntax as outputted by create_mask_range.py are applied to the chosen volumes
and projections created. Multiple volumes and parameter sets can be specified.
...
import numpy as np
import os, sys, traceback, psutil, re
import argparse
...
class Ui_MainWindow(object):
    ...
    def fluo_project_multiple(self,paramFiles=None,cfFiles=None,maskFile=None,save=False):
        foo = os.path.basename(maskFile).split('_mask_plane')
        prefix = '_mask_plane'.join(foo[:-1])
        filepattern = re.compile(rf'^{prefix}_mask_plane([+,-]\d+).tif$')
        maskFiles = [os.path.join(self.workdir,f) for f in os.listdir(self.workdir) if re.search(filepattern, f)]
        for paramFile in paramFiles:
            ...
            for maskFile in maskFiles:
                ...
                match = re.search(filepattern, os.path.basename(maskFile))
                plane_string = match.group(1)
                proj_stack = np.zeros([len(cfFiles),param.nY,param.nX])
                for chNum, cfFile in enumerate(cfFiles):
                    ...
                    proj_stack[chNum,:,:] = CFproj
                prefix = '.'.join(os.path.basename(paramFile).split('.')[0:-1])
                suffix = '.'.join(os.path.basename(cfFile).split('.')[0:-1])+'_masked_plane'+plane_string
                savepath = os.path.normpath(os.path.join(param.workdir,'_'.join([prefix,suffix,'proj.tif'])))
                tifffile.imwrite(savepath,proj_stack,photometric='minisblack',compress=0)
                print('Composite projection image saved as: '+savepath)
