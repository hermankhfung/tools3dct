...
'''Create a series of masks to a fluorescence volume based on correlated FIB and SEM images

Run 'python3 create_mask_range.py --help' for command line usage information.

A series of slices centred 0 to n px away from a user-specified height in the FIB image are created.
Use in combination with project_fluo_range.py to apply masks and generate projections in series.
...
class Ui_MainWindow(object):
    ...
    def __init__(self,fibFile=None,fibparamFile=None,semFile=None,semparamFile=None,lmThickness=None,cfPxSize=None,cfFile=None,search_range=0):
    ...
    def create_mask(self):
        ...
        if self.nX and self.cfPxSize and self.lmThickness and self.fibParam and self.scene_fib.ellipseitem and (self.flag_skipSEM or (self.semParam and self.scene_sem.polygon.size() > 2 and self.scene_sem.ellipseitem_interior)):
            ...
            fibpts = self.get_scenepos([self.scene_fib.ellipseitem])
            if not self.flag_skipSEM:
                sempts = [ [pt.x(), pt.y()] for pt in self.scene_sem.polygon ]
                semIpts = self.get_scenepos([self.scene_sem.ellipseitem_interior])

            print(f'x: {fibpts[0,0]:.1f}, y: {fibpts[0,1]:.1f}')

            for plane_num in np.arange(-self.search_range, self.search_range+1):

                newfibpts = fibpts.copy()
                newfibpts[:,1] -= plane_num
                IBnorm, IBpt = find_IB_plane(newfibpts,self.fibParam)

                if not self.flag_skipSEM:
                    EBcnrs = find_EB_points_on_plane(sempts,self.semParam,IBnorm,IBpt)
                    EBpt = find_EB_points_on_plane(semIpts,self.semParam,IBnorm,IBpt)[0]
                else:
                    EBcnrs = None
                    EBpt = None
                ...
                # Write mask
                savepath = os.path.normpath(os.path.join(self.workdir,'.'.join(os.path.basename(self.cfFile).split('.')[0:-1])+'_mask_plane'+f'{plane_num:+}'+'.tif'))
...
class QMainWindowCustom(QtWidgets.QMainWindow):
    def __init__(self,parent=None,fibFile=None,fibparamFile=None,semFile=None,semparamFile=None,lmThickness=None,cfPxSize=None,cfFile=None,search_range=0):
        super().__init__(parent)
        self.ui = Ui_MainWindow(fibFile,fibparamFile,semFile,semparamFile,lmThickness,cfPxSize,cfFile,search_range)
...
def create_mask_GUI(fibFile=None,fibparamFile=None,semFile=None,semparamFile=None,lmThickness=None,cfPxSize=None,cfFile=None,search_range=0,parentWidget=None):
    MainWindow = QMainWindowCustom(parentWidget,fibFile,fibparamFile,semFile,semparamFile,lmThickness,cfPxSize,cfFile,search_range)
    return MainWindow

if __name__ == "__main__":

    parser = argparse.ArgumentParser(description='Create (and apply) mask to fluorescence volume based on FIB and SEM images')
    ...
    parser.add_argument('--fluo', dest='cfFile', metavar='FILE', help='fluorescence volume')
    parser.add_argument('--range', dest='search_range', required=True, metavar='INT', help='Y-range for which to generate slices')
    ...
    args.search_range = int(args.search_range)

    app = QtWidgets.QApplication(sys.argv)
    MainWindow = create_mask_GUI(args.fibFile,args.fibparamFile,args.semFile,args.semparamFile,args.lmThickness,args.cfPxSize,args.cfFile,args.search_range)
    sys.exit(app.exec_())
